...
class MACNetwork(nn.Module, Default):
    def __init__(self, args):
        super(MACNetwork, self).__init__()
        self.init_encoders(args)
        max_step = args.mac_step
        dim = args.mac_hidden
        self_attention = args.mac_sa
        memory_gate = args.mac_mg
        classes = args.a_size
        dropout = args.mac_dropout

        self.visual_encoder = nn.Sequential(nn.Conv2d(1024, dim, 3, padding=1),
                                nn.ELU(),
    ...
    def reset(self):
        ...
        kaiming_uniform_(self.visual_encoder[0].weight)
    ...
    def forward(self, image, question, question_len):
        ...
        lstm_out, h = self.text_encoder(question, question_len)
        # img = self.conv(image)
        #
